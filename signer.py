...
class signer:
    ...
    def Sign(self, FilePath, PrivateKeyFilePath, OutputFilePath):
        Content = None
        PrivateKey = None
        try:
        ...
        try:
            signature = rsa.sign(Content, PrivateKey, "SHA-256")
    ...
    def Verify(self, FilePath, SignatureFilePath, PublicKeyFilePath):
        ...
        try:
            Verification = rsa.verify(Content, Signature, PublicKey)
    ...
    def SetArgs(self):
        ...
        SignGroupOptions = {"sign", "output", "priv_key"}
...
if __name__ == "__main__":
    ...
    s.SetArgs()
    if s.args.sign:
        FilePath = s.args.sign
        PrivateKeyFilePath = s.args.priv_key
        OutputSignFilePath = s.args.output
        ...
        s.Sign(FilePath, PrivateKeyFilePath, OutputSignFilePath)

    if s.args.verify:
        FilePath = s.args.verify
        PublicKeyFilePath = s.args.pub_key
        SignatureFilePath = s.args.signature_file
    ...
    if s.args.generate_key:
        KeyPairID = s.args.generate_key
        DirPath = s.args.dir
        KeySize = s.args.key_size
